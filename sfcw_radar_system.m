...
%}
%
[R01, d_01, R0, R1, d_R0, d_R1, handles.x_tick, handles.y_tick, handles.z_tick, handles.x_grid, handles.y_grid, handles.z_grid, index_mat, phase_mat] =...
    kir_backward_calculate_3d_image_grid(handles.data_info.fl, dr, handles.data_info.antenna_pos, handles.image_info, ifft_number);%Kirchhoff backward
...
for i=1:frame_number
    ...
    [bp_sum, bp_sum_cf] = back_projection_3d(...
        ...
        %}
    [bp_sum, bp_sum_cf] = kirchhoff_backward_3d(R01, d_01, R0, R1, d_R0, d_R1, calibrated_range_profiles, index_mat, phase_mat);%Kirchhoff backward
...
switch data_type_text
    ...
    case {'杂波抑制'}
        ...
        data = complex(fread(fid1, [ifft_number, tr_pair_number], 'double'),...
            fread(fid1, [ifft_number, tr_pair_number], 'double'));
        %show_channel_figures(handles, 1);
        show_channel_figures(handles, channel_index);
        data_abs = abs(data(:,channel_index-ref_channel_number));
        db_data = 20*log10(data_abs/max(data_abs));
        x_tick = (0:ifft_number-1)/ifft_number*handles.ru;
        ...
        fclose(fid1);
    case {'成像'}%cx
        frame_count_1 = round(get(handles.slider_frame_count,'Value'));
        fid1 = fopen('bp_sum_cf.dat','rb');
        [x, y, z] = size(handles.x_grid);
        fseek(fid1, (frame_count_1-1)*x*y*z*8, 'bof');
        data = reshape(fread(fid1, x*y*z, 'double'), [x,y,z]);
        data=abs(data);
        fclose(fid1);
        ...
        image=data(:,:,31);
        [b,a]=size(image);
        image=reshape(image,b,a);
        image=image(54,:);
        image_data_abs = abs(image);
        image_data_dB = 20*log10( image_data_abs/max(image_data_abs(:)) );
        plot(handles.x_tick, image_data_dB); 
        grid on;ylabel('|H|/dB');xlabel('X/m');
       ...
       % 距离向剖面曲线
        ...
        image_data_abs = abs(image);
        image_data_dB = 20*log10( image_data_abs/max(image_data_abs(:)) );
        plot(handles.y_tick, image_data_dB); 
        grid on;ylabel('|H|/dB');xlabel('Y/m');
        title(['第',num2str(handles.frame_count),'帧 ',...
            'Y Projiection Curve']);
%     case {'慢时间维频谱'}
